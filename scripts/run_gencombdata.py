...
ftemp = join(dumpdir, "temp",
             "gencomb_{}_{}".format(args.reg, args.features) + "_{}.npz")
fperm = join("../results/gencomb", "{}_{}.p".format(args.reg, args.features))
...
def do_task(n):
    ...
    _X, y, __ = frame.make_Xy(
        target=(alpha, beta), features=features,
        append_variances=True if args.reg == "NN" else False)
    # Loop over the splits
    _scores = numpy.full(args.n_splits, numpy.nan)
    _imp = numpy.full((args.n_splits, nfeatures, 3), numpy.nan)
    for i in range(args.n_splits):
        ...
        if args.reg == "NN":
            ...
            _scores[i] = reg.score(X[test], y[test]) / test.size
        elif args.reg == "ET":
...
comm.Barrier()
if rank == 0:
    scores = numpy.full((ngrid, args.n_splits), numpy.nan)
    imps = numpy.full((ngrid, args.n_splits, nfeatures, 3), numpy.nan)
